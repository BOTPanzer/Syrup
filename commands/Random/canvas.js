...
exports.run = (client, message, args, func, prefix) => {
    if (channels.includes(message.channel.id)) {
        message.reply("No puedes tener más de un canvas en un canal.");
        return;
    }
    var size;
    if (+args[0] >= 5 && +args[0] <= 50) {
        ...
        message.reply("Tamaño entre 5 y 50.");
        return;
    }
    message.channel.send("Cargando...").then((msg) => {
...
class Drawing {
    constructor(msg, size, fg, bg) {
        ...
        msg.edit("Reacciona para mover el boli :\n✅ Parar de dibujar | 🖋 Dibujar | 🔏 Levantar boli");
        this.reactArrows(0);
    ...
    stop(reason = "") {
        ...
        this.msg.edit("Se ha parado de dibujar." + reason);
    ...
    renewTimeout() {
        ...
        this.timeout = this.msg.client.setTimeout(function() {
            self.stop("\nRazón de finalización : Fuera de tiempo (2 minutos)");
        }, 120000);
    }
    ...
    handleReaction(reaction) {
        ...
        reaction.remove(reaction.users.last()).catch(e => {
            if (e.code === 50013) reaction.message.channel.send("Necesito permiso de 'Administrar mensajes' para funcionar.");
        });
        this.drawCanvas();
    }
